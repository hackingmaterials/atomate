...
from matmethods.vasp.drones import VaspToDbTaskDrone_to_remove, VaspDrone
...
@explicit_serialize
class VaspToDbTask_to_remove(FireTaskBase):
    """
    Enter a VASP run into the database. By default, the VASP directory is
    assumed to be the current directory.
    ...
    def run_task(self, fw_spec):
        ...
        if not db_file:
            drone = VaspToDbTaskDrone_to_remove(simulate_mode=True)
            task_doc = drone.get_task_doc(calc_dir)
            with open("task.json", "w") as f:
                f.write(json.dumps(task_doc, default=DATETIME_HANDLER))
        else:
            d = get_settings(db_file)
            drone = VaspToDbTaskDrone_to_remove(host=d["host"], port=d["port"],
                                                database=d["database"],
