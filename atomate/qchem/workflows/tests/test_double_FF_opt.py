...
class TestDoubleFFOpt(AtomateTest):
    def test_double_FF_opt(self):
        ...
        test_double_FF_files = os.path.join(
            module_dir, "..", "..", "test_files", "double_FF_wf"
        )
        # define starting molecule and workflow object
        initial_qcin = QCInput.from_file(
            os.path.join(
                test_double_FF_files, "block", "launcher_first", "mol.qin.opt_0"
            )
        )
        ...
        ref_dirs = {
            "first_FF_no_pcm": os.path.join(
                test_double_FF_files, "block", "launcher_first"
            ),
            "second_FF_with_pcm": os.path.join(
                test_double_FF_files, "block", "launcher_second"
            ),
        }
        ...
        self.assertEqual(
            second_FF["calcs_reversed"][0]["input"]["solvent"], {"dielectric": "10.0"}
        )
