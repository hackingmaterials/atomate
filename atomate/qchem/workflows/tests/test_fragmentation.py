...
class TestFragmentation(AtomateTest):
    def test_Fragmentation(self):
        with patch("atomate.qchem.firetasks.fragmenter.FWAction") as FWAction_patch:
            ...
            test_FF_then_fragment_files = os.path.join(
                module_dir, "..", "..", "test_files", "FF_then_fragment_wf"
            )
            ...
            ref_dirs = {
                "first FF": os.path.join(
                    test_FF_then_fragment_files, "block", "launcher_first"
                ),
                "fragment and FF_opt": os.path.join(
                    test_FF_then_fragment_files, "block", "launcher_second"
                ),
            }
            ...
            first_FF = self.get_task_collection().find_one({"task_label": "first FF"})
    ...
    def test_no_opt_Fragmentation(self):
        ...
        with patch("atomate.qchem.firetasks.fragmenter.FWAction") as FWAction_patch:
            ...
            rapidfire(
                self.lp,
                fworker=FWorker(env={"max_cores": 24, "db_file": db_file}),
                pdb_on_exception=True,
            )
