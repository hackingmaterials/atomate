...
class TestFragmentMolecule(AtomateTest):
    ...
    def setUpClass(cls):
        ...
        cls.tfsi = Molecule.from_file(
            os.path.join(module_dir, "..", "..", "test_files", "TFSI.xyz")
        )
        cls.neg_tfsi = Molecule.from_file(
            os.path.join(module_dir, "..", "..", "test_files", "TFSI.xyz")
        )
    ...
    def test_edges_given_TFSI(self):
        with patch("atomate.qchem.firetasks.fragmenter.FWAction") as FWAction_patch:
            ft = FragmentMolecule(
                molecule=self.tfsi, edges=self.tfsi_edges, depth=0, do_triplets=False
            )
            ft.run_task({})
            self.assertEqual(ft.check_db, False)
            self.assertEqual(ft.charges, [-1, 0, 1])
            self.assertEqual(len(FWAction_patch.call_args[1]["additions"]), 468)
    ...
    def test_neg_TFSI_with_additional_charges(self):
        with patch("atomate.qchem.firetasks.fragmenter.FWAction") as FWAction_patch:
            ...
            self.assertEqual(len(FWAction_patch.call_args[1]["additions"]), 624)
    ...
    def test_neg_TFSI(self):
        with patch("atomate.qchem.firetasks.fragmenter.FWAction") as FWAction_patch:
            ...
            self.assertEqual(len(FWAction_patch.call_args[1]["additions"]), 312)

    def test_build_unique_relevant_molecules(self):
        ft = FragmentMolecule(
            molecule=self.pc, edges=self.pc_edges, depth=0, opt_steps=1000
        )
        ...
        loadfn(os.path.join(module_dir, "pc_mols.json"))
        # self.assertEqual(ft.unique_molecules, ref_mols)
        ft = FragmentMolecule(
            molecule=self.pos_pc, edges=self.pc_edges, depth=0, opt_steps=1000
        )
        ft.mol = ft.get("molecule")
        ft.depth = ft.get("depth")
        ft.charges = [-1, 0, 1, 2]
        ...
        loadfn(os.path.join(module_dir, "pos_pc_mols.json"))
        # self.assertEqual(ft.unique_molecules, ref_mols)
        ...
        loadfn(os.path.join(module_dir, "pc_frag1_mols.json"))
        # self.assertEqual(ft.unique_molecules, ref_mols)
    ...
    def test_in_database_and_EC_neg_frag(self):
        ...
        with patch("atomate.qchem.firetasks.fragmenter.FWAction") as FWAction_patch:
            ft = FragmentMolecule(
                molecule=self.neg_ec,
                depth=1,
                qchem_input_params={"pcm_dielectric": 40.0},
                check_db=True,
                db_file=db_file,
            )
            ft.run_task({})
    ...
    def test_in_database_with_actual_database(self):
        ...
        mol2620 = QCOutput(os.path.join(dir2620, "mol.qout.opt_0")).data[
            "initial_molecule"
        ]
        ...
        with patch("atomate.qchem.firetasks.fragmenter.FWAction") as FWAction_patch:
            ...
            self.assertEqual(ft.check_db, True)
            self.assertEqual(ft.charges, [-1, 0, -2, 1])
            self.assertEqual(len(FWAction_patch.call_args[1]["additions"]), 623)
    ...
    def test_babel_PC_with_RO_depth_0_vs_depth_10(self):
        with patch("atomate.qchem.firetasks.fragmenter.FWAction") as FWAction_patch:
            ...
            self.assertEqual(len(FWAction_patch.call_args[1]["additions"]), 1527)
    ...
    def test_babel_PC_depth_0_vs_depth_10(self):
        with patch("atomate.qchem.firetasks.fragmenter.FWAction") as FWAction_patch:
            ft = FragmentMolecule(
                molecule=self.pc, depth=0, open_rings=False, do_triplets=False
            )
            ft.run_task({})
            self.assertEqual(ft.check_db, False)
            depth0frags = ft.unique_fragments
        ...
        with patch("atomate.qchem.firetasks.fragmenter.FWAction") as FWAction_patch:
            ft = FragmentMolecule(
                molecule=self.pc, depth=10, open_rings=False, do_triplets=False
            )
            ft.run_task({})
            self.assertEqual(ft.check_db, False)
            depth10frags = ft.unique_fragments
            self.assertEqual(len(depth10frags), 63)
            self.assertEqual(len(FWAction_patch.call_args[1]["additions"]), 63 * 3)
