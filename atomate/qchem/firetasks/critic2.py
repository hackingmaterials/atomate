...
class ProcessCritic2(FiretaskBase):
    ...
    def run_task(self, fw_spec):
        ...
        for cpid in bond_dict:
            ...
            if bond_dict[cpid]["atoms"] == ["Li", "C"] or bond_dict[cpid]["atoms"] == [
                "C",
                "Li",
            ]:
                if (
                    bond_dict[cpid]["field"] > 0.012
                    and bond_dict[cpid]["distance"] < 2.5
                ):
                    bonds.append(
                        [int(entry) - 1 for entry in bond_dict[cpid]["atom_ids"]]
                    )
