...
def get_wf_xas(
    ...
    spectrum_type = get_feff_input_set_obj(
        feff_input_set, ab_atom_indices[0], structure
    ).__class__.__name__[2:-3]
    ...
    for ab_idx in ab_atom_indices:
        ...
        fw_name = f"{spectrum_type}-{edge}-{ab_idx}"
        fws.append(
            XASFW(
                ab_idx,
                structure,
