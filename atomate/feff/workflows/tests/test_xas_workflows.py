...
class TestXASWorkflow(AtomateTest):
    def setUp(self):
        ...
        self.structure = Structure.from_file(
            os.path.join(module_dir, "..", "..", "test_files", "Co2O2.cif")
        )
        # PymatgenTest.get_mp_structure("mp-715460")
        self.user_tag_settings = {
            "RPATH": -1,
            "SCF": "7 0 30 0.2 3",
            "FMS": "9 0",
            "LDOS": "-30.0 30.0 0.1",
    ...
    def test_xanes_vs_exafs(self):
        ...
        wf_exafs = get_wf_xas(
            self.absorbing_atom, self.structure, feff_input_set="EXAFS", edge="K"
        )

        self.assertEqual(
            wf_xanes.as_dict()["fws"][0]["spec"]["_tasks"][0]["feff_input_set"][
                "@class"
            ],
            "MPXANESSet",
        )
        self.assertEqual(
            wf_exafs.as_dict()["fws"][0]["spec"]["_tasks"][0]["feff_input_set"][
                "@class"
            ],
            "MPEXAFSSet",
        )
