...
class TestLammpsWorkflows(AtomateTest):
    def setUp(self):
        ...
        self.input_file_template = os.path.join(
            module_dir, "test_files/peo.in.template"
        )
        self.user_settings = {
        ...
        self.reference_files_path = os.path.abspath(
    ...
    def test_lammps_wflow(self):
        ...
        wf = get_wf_basic(
            ...
            dump_filenames=self.dump_filenames,
            name="peo_wflow_test",
    ...
    def _check_run(self, d):
        if not LAMMPS_CMD:
            ...
            self.assertTrue(
                filecmp.cmp(
                    os.path.join(path, "peo.in"), f"{self.reference_files_path}/peo.in"
                )
            )
            self.assertTrue(
                filecmp.cmp(
                    os.path.join(path, "peo.log"),
                    f"{self.reference_files_path}/peo.log",
                )
            )
            self.assertTrue(
                filecmp.cmp(
                    os.path.join(path, "peo.dump"),
                    f"{self.reference_files_path}/peo.dump",
                )
            )
            self.assertTrue(
                filecmp.cmp(
                    os.path.join(path, "peo.dcd"),
