...
class BandgapEstimationBuilder:
    ...
    def run(self):
        ...
        q = {
            "dielectric.epsilon_static_avg": {"$gt": 0},
            "bandgap_estimation": {"$exists": False},
        }
    ...
    def reset(self):
        logger.info(f"Resetting {self.__class__.__name__} starting!")
        self._materials.update_many({}, {"$unset": {"bandgap_estimation": 1}})
        logger.info(f"Resetting {self.__class__.__name__} finished!")
