...
class PathfinderImagesToDb(FiretaskBase):
    ...
    def run_task(self, fw_spec):
        ...
        approx_neb_doc = mmdb.collection.find_one({"wf_uuid":wf_uuid},{"stable_sites":1, "host_lattice.task_id":1, "_id": 0})
        ...
        end_point_structs = [Structure.from_dict(stable_site["output"]["structure"]) for stable_site in stable_sites]

        # checks if all indexes match and if so, gets the inserted site indexes
        indexes = [i for i in stable_site["inserted_site_indexes"]]
        if all([i==indexes[0] for i in indexes]): #if all indexes match
            inserted_site_indexes = indexes[0]
        else:
            ValueError("Inserted site indexes of end point structures must match for NEBPathfinder")

        # applies NEBPathFinder to interpolate and get images
        if len(end_point_structs) == 2:
            neb_pf = NEBPathfinder(end_point_structs[0],end_point_structs[-1],relax_sites = inserted_site_indexes,v = host_lattice_v,n_images=n_images-1)
            #note NEBPathfinder currently returns n_images+1 images (rather than n_images)
        else:
            ValueError("Firetask does not support NEBPathfinder with more than two stable sites")
            #ToDo: handling more than two provided stable sites

        # stores images generated by NEBPathFinder in approx_neb collection
        mmdb.collection.update_one({"wf_uuid": wf_uuid}, {"$set": {"images":neb_pf.images}})
