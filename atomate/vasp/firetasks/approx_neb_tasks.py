...
class PathfinderToDb(FiretaskBase):
    ...
    def run_task(self, fw_spec):
        ...
        pathfinder_output = {"images":neb_pf.images,
...
class AddSelectiveDynamics(FiretaskBase):
    ...
    required_params = ["db_file", "approx_neb_wf_uuid", "mobile_specie", "selective_dynamics_scheme"]
    optional_params = []
    ...
    def run_task(self, fw_spec):
        ...
        wf_uuid = self.get("approx_neb_wf_uuid")
        approx_neb_doc = mmdb.collection.find_one({"wf_uuid": wf_uuid},{"pathfinder":1})
        ...
        if scheme == "fix_two_atom":
            images = [self.add_fix_two_atom_selective_dynamics(structure, fixed_index, fixed_specie) for structure in structures]
        else:
            pass
        ...
        for n,image in enumerate(images):
            images_output.append ({"index":n,
                                   "input_structure":image.as_dict(),
                                   "selective_dynamics_scheme":scheme})

        mmdb.collection.update_one({"wf_uuid": wf_uuid},
                                   {"$set":  {"images": images_output}})

        return FWAction(update_spec={"images": images_output},
                        stored_data={"wf_uuid": wf_uuid,
