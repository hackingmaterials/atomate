...
class TestElasticWorkflow(AtomateTest):
    ...
    def _check_run(self, d, mode):
        ...
        if mode in ["structure optimization"]:
            self.assertAlmostEqual(
                d["calcs_reversed"][0]["output"]["structure"]["lattice"]["a"], 5.469, 2
            )
            ...
            np.testing.assert_allclose(
                [c_ij[0, 0], c_ij[0, 1], c_ij[3, 3]], [146.25, 52.26, 75.23], rtol=1e-2
            )
            ...
            np.testing.assert_allclose(
                [c_ij[0, 0, 0], c_ij[0, 0, 1], c_ij[0, 1, 2]],
                [-779.6, -432.5, -93.3],
                rtol=1e-1,
            )
    ...
    def test_wf(self):
        ...
        defo_vis = [
            fw.tasks[2]["vasp_input_set"]
            for fw in self.preset_wf.fws
            if "deform" in fw.name
        ]
        assert all([vis.user_incar_settings["ENCUT"] == 700 for vis in defo_vis])
        assert all(
            [vis.user_kpoints_settings.get("grid_density") == 7000 for vis in defo_vis]
        )
