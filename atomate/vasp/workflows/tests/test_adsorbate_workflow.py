...
class TestAdsorptionWorkflow(AtomateTest):
    ...
    def test_wf_functions(self):
        ...
        for slab in self.slabs:
            ...
            self.assertTrue(np.allclose(new_slab.lattice.matrix, slab.lattice.matrix))
        ...
        for struct in structs:
            slabs = generate_all_slabs(
                struct, max_index=2, min_slab_size=10, min_vacuum_size=20
            )
            for slab in slabs:
                trans_params = get_slab_trans_params(slab)
                trans = SlabTransformation(**trans_params)
    ...
    def _simulate_vasprun(self, wf):
        ...
        ir_ref_dirs = {
            "Ir-structure optimization": os.path.join(reference_dir, "1"),
            "Ir-Ir_(1, 0, 0) slab optimization": os.path.join(reference_dir, "2"),
            "Ir-H1-Ir_(1, 0, 0) adsorbate optimization 0": os.path.join(
                reference_dir, "3"
            ),
            "Ir-H1-Ir_(1, 0, 0) adsorbate optimization 1": os.path.join(
                reference_dir, "4"
            ),
            "Ir-H1-Ir_(1, 0, 0) adsorbate optimization 2": os.path.join(
                reference_dir, "5"
            ),
        }
        return use_fake_vasp(wf, ir_ref_dirs, params_to_check=["ENCUT", "ISIF"])
    ...
    def test_wf(self):
        ...
        ads_vis = [
            fw.tasks[1]["vasp_input_set"]
            for fw in self.wf_1.fws
