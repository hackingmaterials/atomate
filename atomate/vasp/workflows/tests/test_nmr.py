...
class TestNMRWorkflow(AtomateTest):
    ...
    def _simulate_vasprun(self, wf):
        ref_dirs = {
            "structure optimization": os.path.join(
                reference_dir, "structure_optimization"
            ),
    ...
    def _check_run(self, d, mode):
        ...
        if mode == "structure optimization":
            self.assertAlmostEqual(
                d["calcs_reversed"][0]["output"]["structure"]["lattice"]["a"], 5.297, 2
            )
        ...
        if mode == "cs tensor":
            ...
            np.testing.assert_allclose(
                cs["g0"],
                [
                    [-3.896026, 0.008131, 0.004022],
                    [0.010171, -4.303434, 0.007171],
                    [0.013091, 0.007672, -4.478203],
                ],
            )
            np.testing.assert_allclose(
                cs["raw"][0],
                [
                    [-56.972805, -0.000285, -0.000021],
                    [-0.000354, -47.358647, 0.000055],
            ...
            np.testing.assert_allclose(
                cs["raw"][-1],
                [
                    [-13.001104, -20.568634, 19.91829],
                    [-25.384874, -37.953075, 28.781717],
