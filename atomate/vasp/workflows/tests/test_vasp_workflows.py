...
class TestVaspWorkflows(AtomateTest):
    ...
    def _check_run(self, d, mode):
        ...
        if mode == "additional field":
            self.assertAlmostEqual(
                d["test_additional_field"]["lattice"]["a"], 3.8401979337
            )
        ...
        if "nscf" in mode:
            self.assertEqual(
                d["calcs_reversed"][0]["output"]["outcar"]["total_magnetization"], None
            )
        else:
            self.assertAlmostEqual(
                d["calcs_reversed"][0]["output"]["outcar"]["total_magnetization"], 0, 3
            )

        self.assertLess(
            d["run_stats"]["overall"]["Elapsed time (sec)"], 180
        )  # run should take under 3 minutes
    ...
    def test_bandstructure_Vasp(self):
        ...
        d = self.get_task_collection().find_one(
            {"task_label": "structure optimization"}, sort=[("_id", DESCENDING)]
        )
        self._check_run(d, mode="structure optimization")

        # make sure the static run ran OK
        d = self.get_task_collection().find_one(
            {"task_label": "static"}, sort=[("_id", DESCENDING)]
        )
        ...
        # make sure the uniform run ran OK
        d = self.get_task_collection().find_one(
            {"task_label": "nscf line"}, sort=[("_id", DESCENDING)]
        )
    ...
    def test_bandgap_check_Vasp(self):
        ...
        self.assertEqual(
            self.lp.fireworks.find_one(
                {"name": "Si-structure optimization"}, {"state": 1}
            )["state"],
            "COMPLETED",
        )

        self.assertEqual(
            self.lp.fireworks.find_one({"name": "Si-static"}, {"state": 1})["state"],
            "DEFUSED",
        )
    ...
    def test_chgcar_db_read_write(self):
        ...
        self.assertAlmostEqual(
            cc.data["total"].sum() / cc.ngridpts, 23.253588293583313, 4
        )
        cc = decoder.process_decoded(doc["calcs_reversed"][0]["aeccar2"])
        self.assertAlmostEqual(
            cc.data["total"].sum() / cc.ngridpts, 8.01314480789829, 4
        )
        ...
        self.assertAlmostEqual(
            dcc["aeccar0"].data["total"].sum() / cc.ngridpts, 23.253588293583313, 4
        )
        self.assertAlmostEqual(
            dcc["aeccar2"].data["total"].sum() / cc.ngridpts, 8.01314480789829, 4
        )
        ...
        self.assertAlmostEqual(
            ret_chgcar.data["total"].sum() / ret_chgcar.ngridpts, 8.0, 4
        )
        self.assertAlmostEqual(
            ret_aeccar.data["total"].sum() / ret_aeccar.ngridpts, 31.2667331015, 4
        )
...
class TestScanOptimizeWorkflow(AtomateTest):
    ...
    def _run_scan_relax(self, wf, formula):
        if not VASP_CMD:
            wf = use_fake_vasp(
                wf,
                {
                    "PBEsol structure optimization": os.path.join(
                        reference_dir, f"PBESol_pre_opt_for_SCAN_{formula}"
                    ),
                    "R2SCAN structure optimization": os.path.join(
                        reference_dir, f"SCAN_structure_optimization_{formula}"
                    ),
                },
                check_kpoints=False,
                check_potcar=False,
    ...
    def _get_launch_dir(self):
        ...
        pbesol = list(
            self.get_task_collection().find(
                {"task_label": "PBEsol structure optimization"}
            )
        )[-1]
        r2scan = list(
            self.get_task_collection().find(
                {"task_label": "R2SCAN structure optimization"}
            )
        )[-1]
    ...
    def test_SCAN_no_bandgap(self):
        ...
        structure = Structure.from_file(
            os.path.join(reference_dir, "PBESol_pre_opt_for_SCAN_Al/inputs", "POSCAR")
        )
        ...
        for p in incar:
            if p == "KSPACING":
                ...
                p == "MAGMOM"
            ):  # Ignore MAGMOM b/c structure initialized from POSCAR cannot have a MAGMOM
                pass
            else:
        ...
        ref_incar = Incar.from_file(
            os.path.join(
                reference_dir, "SCAN_structure_optimization_Al/inputs", "INCAR"
            )
        )
    ...
    def test_SCAN_small_bandgap(self):
        ...
        ref_incar = Incar.from_file(
            os.path.join(
                reference_dir, "SCAN_structure_optimization_LiH/inputs", "INCAR"
            )
        )
        incar = Incar.from_file(os.path.join(self._get_launch_dir()[1], "INCAR.gz"))
        for p in incar:
    ...
    def test_SCAN_large_bandgap(self):
        ...
        structure = Structure.from_file(
            os.path.join(reference_dir, "PBESol_pre_opt_for_SCAN_LiF/inputs", "POSCAR")
        )
        ...
        ref_incar = Incar.from_file(
            os.path.join(
                reference_dir, "SCAN_structure_optimization_LiF/inputs", "INCAR"
            )
        )
        incar = Incar.from_file(os.path.join(self._get_launch_dir()[1], "INCAR.gz"))
        for p in incar:
    ...
    def test_SCAN_with_vdw(self):
        ...
        structure = Structure.from_file(
            os.path.join(
                reference_dir, "PBESol_pre_opt_for_SCAN_LiF_vdw/inputs", "POSCAR"
            )
        )
        ...
        my_wf = get_wf(
            ...
            common_params={
                ...
                "vdw_kernel_dir": os.path.join(
                    reference_dir, "PBESol_pre_opt_for_SCAN_LiF_vdw/inputs"
                ),
            },
        )
        ...
        ref_incar = Incar.from_file(
            os.path.join(
                reference_dir, "PBESol_pre_opt_for_SCAN_LiF_vdw/inputs", "INCAR"
            )
        )
        incar = Incar.from_file(os.path.join(self._get_launch_dir()[0], "INCAR.gz"))
        ...
        ref_incar = Incar.from_file(
            os.path.join(
                reference_dir, "SCAN_structure_optimization_LiF_vdw/inputs", "INCAR"
            )
        )
        incar = Incar.from_file(os.path.join(self._get_launch_dir()[1], "INCAR.gz"))
