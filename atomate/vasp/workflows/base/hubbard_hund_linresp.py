...
def get_wf_hubbard_hund_linresp(
    ...
    vis_gs = HubbardHundLinRespSet(
        structure=structure, num_perturb=num_perturb, **vis_params.copy())
    fws.append(HubbardHundLinRespFW(
        structure=structure, name="initial_static",
    ...
    if relax_nonmagnetic:
        ...
        vis_gs = HubbardHundLinRespSet(
            structure=structure, num_perturb=num_perturb, **vis_params.copy())
        fws.append(HubbardHundLinRespFW(
            structure=structure, parents=fws[-1],
                        ...
                        for i in range(num_perturb):
                            if i == counter_perturb:
                                val_dict[k].update(
                                    {"perturb"+str(i):
                                     specie = str(
                                         structure[counter_perturb].specie.block)
                                     block_dict[specie]})
                            else:
                                val_dict[k].update({"perturb"+str(i): -1})
                    else:
...
class PoscarPerturb(Poscar):
    ...
    def site_symbols(self):
        ...
        if (self.num_perturb > 0 and self.num_perturb <= len(self.structure)):
            syms = [site.specie.symbol
                    for site in self.structure[self.num_perturb:]]
            syms = [a[0] for a in itertools.groupby(syms)]
            syms_perturb = [site.specie.symbol
                            for site in self.structure[0:self.num_perturb]]
